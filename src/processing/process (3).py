...
# Precomputed once from real USGS elevation data (see slope_lookup.py),
# keyed by grid_cell_id. Falls back to a flat average if a cell is
# somehow missing rather than crashing the run.
print("Loading terrain slope lookup...")
slope_lookup = {}
DEFAULT_SLOPE = 15.0
try:
    s3.download_file(
        BUCKET, "model/slope_lookup.json", "/tmp/slope_lookup.json"
    )
    with open("/tmp/slope_lookup.json") as f:
        slope_lookup = json.load(f)
    if slope_lookup:
        DEFAULT_SLOPE = round(
            sum(slope_lookup.values()) / len(slope_lookup), 2
        )
    print(f"Loaded slope data for {len(slope_lookup)} grid cells")
except Exception as e:
    print(f"Could not load slope lookup, using flat fallback: {e}")
            ...
            with rasterio.open("/tmp/pre_B04.jp2") as pre_b04:
                with rasterio.open("/tmp/pre_B08.jp2") as pre_b08:
                    ...
                    for row_start, col_start in sample_windows:
                        ...
                        grid_cell_id = f"{tile_name}_{row_start}_{col_start}"
                        slope_degrees = slope_lookup.get(
                            grid_cell_id, DEFAULT_SLOPE
                        )
                        ...
                        # TTL: expire this row automatically after ~10 days
                        # so old duplicate readings don't pile up forever
                        ttl_timestamp = int(
                        ...
                        record = {
                            ...
                            "slope_degrees": str(round(slope_degrees, 2)),
